...
import { BookOpen, CheckCircle2, Target, Trash2 } from 'lucide-react'
...
export default function PlanoPage() {
  ...
  const deleteChapter = async (id: string) => {
    await supabase.from('historico_leitura').delete().eq('id', id)
    setHistorico(prev => prev.filter(h => h.id !== id))
  }

  const deleteBook = async (livro_en: string) => {
    if (!user) return
    if (!confirm('Remover todo o histórico deste livro?')) return
    await supabase.from('historico_leitura').delete()
      .eq('user_id', user.id).eq('livro_en', livro_en)
    setHistorico(prev => prev.filter(h => h.livro_en !== livro_en))
  }
  ...
  const readFiltered = booksFiltered.reduce((sum, b) => {
    return sum + Array.from({ length: b.chapters }, (_, i) => i + 1)
  ...
  const recentBooks = (() => {
    ...
    const result: { book: typeof BIBLE_BOOKS[0]; ch: number; lido_em: string; id: string }[] = []
    for (const h of historico.slice(0, 30)) {
      ...
      if (book && !seen.has(`${h.livro_en}-${h.capitulo}`)) {
        ...
        result.push({ book, ch: h.capitulo, lido_em: h.lido_em, id: h.id })
      ...
      <div className="px-6 flex flex-col gap-4 pb-6">
        ...
        {nextUnread === null && (
          <div className="rounded-2xl p-5 text-center" style={{ background: 'linear-gradient(135deg, #059669, #10B981)' }}>
            ...
            <p className="text-white font-extrabold text-lg">Bíblia completa!</p>
            <p className="text-white/80 text-sm mt-1">Parabéns! Você leu toda a Bíblia!</p>
          </div>
        )}
