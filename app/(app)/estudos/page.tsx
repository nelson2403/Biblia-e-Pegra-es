            ...
            <div className="absolute right-0 top-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg z-10 overflow-hidden w-40">
              {(Object.keys(SORT_LABELS) as SortKey[]).map(k => (
                <button key={k} onClick={() => onSort(k)}
                  ...
                  style={{ color: sort === k ? '#4F46E5' : '#374151' }}>
                  {SORT_LABELS[k]}
                </button>
              ))}
            </div>
            ...
            <div key={item.id} className="bg-white rounded-2xl p-4 shadow-sm">
              <div className="flex items-start gap-3">
                ...
                <Link href={`/estudos/${item.id}`} className="flex-1 min-w-0">
                  <p className="font-bold text-gray-800 text-sm">{item.livro} {item.capitulo}:{item.versiculo}</p>
                  <p className="text-xs text-gray-500 line-clamp-2 mt-0.5">{item.texto_biblico}</p>
                  {item.tags && (
                ...
                <div className="flex gap-1 flex-shrink-0">
                  <Link href={`/estudos/novo?id=${item.id}`} className="p-2 rounded-xl hover:bg-gray-100 text-primary">
                    <Pencil size={17} />
                  </Link>
              ...
              <div className="mt-3 pt-3 border-t border-gray-100">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs text-gray-400">{done === 4 ? '✅ Completo' : `${done}/4 secoes preenchidas`}</span>
                  <span className="text-xs font-bold" style={{ color: done === 4 ? '#059669' : '#4F46E5' }}>{pct.toFixed(0)}%</span>
                </div>
                <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                  <div className="h-1.5 rounded-full transition-all"
                    style={{ width: `${pct}%`, backgroundColor: done === 4 ? '#059669' : '#4F46E5' }} />
                </div>
              </div>
            </div>
          )
